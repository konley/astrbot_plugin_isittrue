...
| `true` | `✅ 真的喵` | 内容属实 |
| `false` | `❌ 假的喵` | 内容不实 |
| `unknown` | `⚠️ 布吉岛` | 无法核实（纯口味 / 私人生活 / 缺关键要素 / 图看不清） |
...
- **feat**: 段子/梗图也抽出里面的可核主张；稳定技术常识允许判 `false`，不再因为「这是玩笑」就 `unknown`
- **feat**: 规划器把玩笑当包装而不是终点；规划误把梗图判成不搜时自动救援
- **feat**: 终判解释放宽到 200 字，要求写清判定对象和机制原因
