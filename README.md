...
群聊事实核查小工具。@机器人说出你想核实的事情，或**引用一条消息**（支持文本 / 图片），AI 自动判断真假，返回 `✅ 真的喵` / `❌ 假的喵` / `⚠️ 布吉岛` + 中文解释。
...
- 命中后统一流程：冷却检测 → 提取内容 → （可选）联网搜索 → 调用 Provider → 返回判定结果 + 中文解释。
...
- **纯图片场景**：开启联网搜索时，若消息只有图片没有文本，会先让多模态模型从图中提取一句可搜索的关键词，再执行搜索。
- **兜底机制**：若 Anysearch 未安装、未启用、搜索失败或超时，自动回退到"仅凭模型知识判断"的原有流程，不会报错中断。

## 错误处理

当 LLM 调用失败时，插件会根据错误类型给出针对性的用户提示，而非统一的"判断失败"：

| 错误类型 | 用户提示 |
|---------|---------|
| 图片内容审核拦截（sensitive / 1026） | 图片内容被AI服务商安全审核拦截，无法判断，请更换图片后重试 |
| 频率限制（429 / rate_limit / quota） | AI服务当前繁忙，请稍后重试 |
| 超时（timeout / timed out） | 判断超时，请稍后重试 |
| 内容超长（context_length / too long） | 内容过长，超出AI处理限制，请精简后重试 |
| 其他异常 | 判断失败，请稍后重试 |
...
- **feat**: 纯图片场景自动提取搜索关键词
- **feat**: 友好错误提示 — 根据 LLM 错误类型（审核拦截 / 频率限制 / 超时等）给出针对性提示，而非统一"判断失败"
- **fix**: 修正判定结果文案（`✅ 真的喵` / `❌ 假的喵`，与代码实际输出一致）
- **docs**: 重写 README，补全配置项、联网搜索、错误处理说明

### v1.0.0

- 初始版本 — 群聊事实核查插件，支持 @触发 / 引用触发 / 前缀后缀监听 / 图片分析
